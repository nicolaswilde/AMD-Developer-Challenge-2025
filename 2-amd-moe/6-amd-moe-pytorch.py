...
CPP_WRAPPER = """
int moe1(torch::Tensor t_input, torch::Tensor t_router,
...
void moe2(torch::Tensor t_input, torch::Tensor t_router,
          torch::Tensor t_gate, torch::Tensor t_up, torch::Tensor t_down,
          torch::Tensor t_output, int dh, int de, int nre, int nse, int ept, int n,
          torch::Tensor t_score_fp32, torch::Tensor t_score_fp16,
          torch::Tensor t_top_indices, torch::Tensor t_token_count,
...
void moe2(torch::Tensor t_input, torch::Tensor t_router,
    ...
    pack_output<<<dim3(n), dim3(64)>>>(output_down, output_ptr, score_fp16, top_indices, expert_offset, index_in_expert, n, ept, nre, nse, dh);
}
"""
...
def custom_kernel(data: input_t) -> output_t:
    ...
    score_fp16 = torch.softmax(input.view(bs*sl, dh) @ router.T, dim=1)  # [bs * sl, nre]

    max_count = module.moe1(input, router, gate, up, down, output,
    ...
    module.moe2(input, router, gate, up, down, output,
        dh, de, nre, nse, ept, bs*sl,
        score_fp32, score_fp16, top_indices, token_count, index_in_expert, expert_offset,
        input_reorder, hidden, output_down)
    return output