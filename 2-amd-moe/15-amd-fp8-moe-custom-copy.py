...
// copy_weights_kernel<<<dim3(96, 128), dim3(64)>>>(ptr);
__global__ void copy_weights_kernel(unsigned long long *ptr) {
    using float4 = float __attribute__((ext_vector_type(4)));
    __half *src_ptr = (__half *)(ptr[blockIdx.x     ]) + blockIdx.y * 16 * 7168;
    __half *dst_ptr = (__half *)(ptr[blockIdx.x + 96]) + blockIdx.y * 16 * 7168;
    for (int i = 0; i < 16; i++) {
        for (int j = threadIdx.x * 8; j < 7168; j += 64 * 8) {
