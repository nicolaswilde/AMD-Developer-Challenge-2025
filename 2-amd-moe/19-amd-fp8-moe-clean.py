...
int moe1(tt t_input, tt t_output, int dh, int de, int nre, int nse, int ept, int n,
         tt t_score, tt t_top_indices, tt t_token_count, tt t_index_in_expert, tt t_input_reorder);
void pack(tt t_input, tt t_output,
          int dh, int de, int nre, int nse, int ept, int n,
          tt t_score, tt t_top_indices, tt t_token_count, tt t_index_in_expert,
...
void copy_weights(
    ...
    tt g0, tt g1, tt g2, tt g3, tt g4, tt g5, tt g6, tt g7, tt g8, tt g9, tt g10, tt g11, tt g12, tt g13, tt g14, tt g15,
    tt g16, tt g17, tt g18, tt g19, tt g20, tt g21, tt g22, tt g23, tt g24, tt g25, tt g26, tt g27, tt g28, tt g29, tt g30, tt g31,
    tt u0, tt u1, tt u2, tt u3, tt u4, tt u5, tt u6, tt u7, tt u8, tt u9, tt u10, tt u11, tt u12, tt u13, tt u14, tt u15,
    tt u16, tt u17, tt u18, tt u19, tt u20, tt u21, tt u22, tt u23, tt u24, tt u25, tt u26, tt u27, tt u28, tt u29, tt u30, tt u31,
    tt d0, tt d1, tt d2, tt d3, tt d4, tt d5, tt d6, tt d7, tt d8, tt d9, tt d10, tt d11, tt d12, tt d13, tt d14, tt d15,
    tt d16, tt d17, tt d18, tt d19, tt d20, tt d21, tt d22, tt d23, tt d24, tt d25, tt d26, tt d27, tt d28, tt d29, tt d30, tt d31);
"""
...
int moe1(tt t_input, tt t_output, int dh, int de, int nre, int nse, int ept, int n,
    ...
    if (nre == 32 && ept == 4 && n % 512 == 0) {
        calculate_index_nre32_ept4<<<dim3(n/512), dim3(512)>>>(top_indices, token_count, index_in_expert, n);
    }
    else {
...
void copy_weights(
    ...
    __half *gate_re_ptr = (__half *)gate_re.data_ptr();
    __half *up_re_ptr   = (__half *)up_re.data_ptr();
    __half *down_re_ptr = (__half *)down_re.data_ptr();
    for (int i = 0; i < nre; i++) ptr[i + 96] = (u64)(&gate_re_ptr[i * dh * de]);
    for (int i = 0; i < nre; i++) ptr[i + 128] = (u64)(&up_re_ptr[i * dh * de]);
    for (int i = 0; i < nre; i++) ptr[i + 160] = (u64)(&down_re_ptr[i * de * dh]);
...
def custom_kernel(data: input_t) -> output_t:
    ...
    gate_re = torch.empty(nre, dh, de, dtype=torch.float16, device='cuda')
    up_re   = torch.empty(nre, dh, de, dtype=torch.float16, device='cuda')
    down_re = torch.empty(nre, de, dh, dtype=torch.float16, device='cuda')
    output = torch.empty(bs, sl, dh, dtype=torch.float16, device='cuda')

    top_indices = torch.empty(bs * sl, ept, dtype=torch.int32, device='cuda')
    token_count = torch.empty(nre, dtype=torch.int32, device='cuda')
    index_in_expert = torch.empty(bs * sl, ept, dtype=torch.int32, device='cuda')
    input_reorder = torch.empty(nre, bs * sl, dh, dtype=torch.float16, device='cuda')
    ...
    gate_se = weights['shared_experts.0.weight'].view(dh, nse, de).transpose(0, 1)
    up_se   = weights['shared_experts.1.weight'].view(dh, nse, de).transpose(0, 1)
    down_se = weights['shared_experts.2.weight'].view(nse, de, dh)

    score = torch.softmax(input.view(bs * sl, dh) @ router.T, dim=1)

    max_count = module.moe1(input, output, dh, de, nre, nse, ept, bs * sl,
        score, top_indices, token_count, index_in_expert, input_reorder)

    hidden_re_gate = torch.bmm(input_reorder[:, :max_count, :], gate_re)
    hidden_re_up   = torch.bmm(input_reorder[:, :max_count, :], up_re)
    module.re_silu_mul(hidden_re_gate, hidden_re_up, nre, max_count, de, token_count)
    hidden_se_gate = torch.matmul(input.view(bs * sl, dh), gate_se)
    hidden_se_up   = torch.matmul(input.view(bs * sl, dh), up_se)
    module.se_silu_mul(hidden_se_gate, hidden_se_up, nse, bs * sl, de)
    output_down_re = torch.bmm(hidden_re_gate, down_re)
    output_down_se = torch.bmm(hidden_se_gate, down_se)
