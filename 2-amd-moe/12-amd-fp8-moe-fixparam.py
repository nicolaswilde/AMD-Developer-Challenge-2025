...
__global__ void pack_output_ept4nre32nse1dh7168(half *output_down, __half *output_ptr, const __half *score, int *top_indices, int *index_in_expert, int n, int ept, int nre, int nse, int dh) {
    int idx = blockIdx.x;
    using half8 = __fp16 __attribute__((ext_vector_type(8)));
    for (int j = threadIdx.x * 8; j < dh; j += 64 * 8) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            __half val[8];
            *(half8 *)val = *(half8 *)&output_down[token_location * 7168 + j];
            for (int k = 0; k < 8; k++) {
                res[k] = __hadd(__hmul(this_score, val[k]), res[k]);
            }
        }
        for (int i = 0; i < 1; i++) {
            __half val[8];
            *(half8 *)val = *(half8 *)&output_down[(n * (32 + i) + idx) * 7168 + j];
            for (int k = 0; k < 8; k++) {
                res[k] = __hadd(val[k], res[k]);
            }
        }
...
void moe2(torch::Tensor t_input, torch::Tensor t_output,
    ...
    if (ept == 4 && nre == 32 && nse == 1 && dh == 7168) {
        pack_output_ept4nre32nse1dh7168<<<dim3(n), dim3(64)>>>(output_down, output_ptr, score, top_indices, index_in_expert, n, ept, nre, nse, dh);
    }
    else {
