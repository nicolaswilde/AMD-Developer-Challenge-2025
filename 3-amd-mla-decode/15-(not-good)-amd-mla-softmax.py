...
__global__ void div_softmax_kernel(__hip_bfloat16 *ptr, int bs, int nh, int sl, float scale) {
    ...
    __shared__ float sum_val[256];

    __hip_bfloat16 val[32];
    __hip_bfloat16 unaligned_val = 0.0f;

    int sl_aligned = sl / 8 * 8;
    int sl_unaligned = sl % 8;

    int this_aligned_num = 0;
    int this_unaligned_num = 0;
    float thread_sum = 0.0f;
    for (int i = 0; i < 4; i++) {
        if (threadIdx.x * 8 + i * 2048 < sl_aligned) {
            *(float4 *)&val[i * 8] = *(float4 *)&ptr[idx + threadIdx.x * 8 + i * 2048];
            this_aligned_num += 8;
        }
    }
    if (sl_aligned + threadIdx.x < sl) {
        unaligned_val = ptr[idx + sl_aligned + threadIdx.x];
        this_unaligned_num = 1;
    }

    for (int i = 0; i < this_aligned_num; i++) {
        float scale_val = (float)val[i] * scale; // Scale by 1/sqrt(dnope + drope)
        float exp_val = expf(scale_val);
        thread_sum += exp_val;
        val[i] = exp_val;
    }
    if (this_unaligned_num > 0) {
        float scale_val = (float)unaligned_val * scale; // Scale by 1/sqrt(dnope + drope)
        float exp_val = expf(scale_val);
        thread_sum += exp_val;
        unaligned_val = exp_val;
    }
    ...
    if (threadIdx.x < 16) {
        ...
        *(float4 *)&this_val[4] = *(float4 *)&sum_val[threadIdx.x * 4 + 64];
        *(float4 *)&this_val[8] = *(float4 *)&sum_val[threadIdx.x * 4 + 128];
        *(float4 *)&this_val[12] = *(float4 *)&sum_val[threadIdx.x * 4 + 192];
        #pragma unroll
        for (int i = 0; i < 16; i++) {
    ...
    if (threadIdx.x == 0) {
        ...
        *(float4 *)&this_val[8] = *(float4 *)&sum_val[8];
        *(float4 *)&this_val[12] = *(float4 *)&sum_val[12];
        #pragma unroll
        for (int i = 0; i < 16; i++) {
            thread_sum += this_val[i];
        }
    ...
    for (int i = 0; i < 4; i++) {
        if (threadIdx.x * 8 + i * 2048 < sl_aligned) {
            #pragma unroll
            for (int j = 0; j < 8; j++) {
                float softmax_val = (float)val[i * 8 + j] / total_sum;
                val[i * 8 + j] = softmax_val;
            }
            *(float4 *)&ptr[idx + threadIdx.x * 8 + i * 2048] = *(float4 *)&val[i * 8];
        }
    }
    if (this_unaligned_num > 0) {
        float softmax_val = (float)unaligned_val / total_sum;
        unaligned_val = softmax_val;
        ptr[idx + sl_aligned + threadIdx.x] = unaligned_val;
    }
}
...
void div_softmax(torch::Tensor qk, int bs, int nh, int sl, int dnope, int drope) {
    ...
    div_softmax_kernel<<<dim3(bs, nh), dim3(256)>>>(qk_ptr, bs, nh, sl, scale);
}
"""
