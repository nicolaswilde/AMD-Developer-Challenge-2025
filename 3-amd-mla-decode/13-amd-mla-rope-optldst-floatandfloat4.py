...
__global__ void rope_3d_kernel_d0aligned4_d2is64(int d0, int d1, int d2, int xs1, int xs2, int ys1, int ys2, int pos,
        const __hip_bfloat16 *x, __hip_bfloat16 *y) {
    // using float2 = float __attribute__((ext_vector_type(2)));
    int d0_idx = blockIdx.x * 4 + threadIdx.x / 16;
    int d1_idx = blockIdx.y;
    int d2_idx = (threadIdx.x % 16) * 2;
    __hip_bfloat16 x1[2], x2[2];
    *(float *)&x1[0] = *(float *)&x[d0_idx * xs1 + d1_idx * xs2 + d2_idx];
    *(float *)&x2[0] = *(float *)&x[d0_idx * xs1 + d1_idx * xs2 + d2_idx + d2/2];
    __hip_bfloat16 idx = __hadd((__hip_bfloat16)pos, __hip_bfloat16(d1_idx));
    __hip_bfloat16 y1[2], y2[2];
    #pragma unroll
    for (int i = 0; i < 2; i++) {
        __hip_bfloat16 theta = (__hip_bfloat16)(d2_idx + i);
        theta = __hmul(theta, (__hip_bfloat16)(-2.0f));
...
void mla_decode(torch::Tensor x, torch::Tensor y, int pos, int xoff, int yoff,
    ...
    else if (d2 == 64 && d0 % 4 == 0) {
        // Use a specialized kernel for 64-dim and d0 multiple of 4
        rope_3d_kernel_d0aligned4_d2is64<<<dim3(d0/4, d1), dim3(64)>>>(d0, d1, d2, xs1, xs2, ys1, ys2, pos, x_ptr, y_ptr);
    }
    else if (d0 % 2 == 0) {
