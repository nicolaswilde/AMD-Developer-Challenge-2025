...
__global__ void custom_kernel(const __hip_fp8_e4m3_fnuz* __restrict__ a,
    ...
    __shared__ __hip_fp8_e4m3_fnuz lds_a[32][136];
    __shared__ __hip_fp8_e4m3_fnuz lds_b[32][136];
    ...
    for (int ib = 0; ib < k/BLOCK; ib++) {
        ...
        for (int ii = 0; ii < 1; ii++) {
            *(float4 *)ldg_a[ii] = *(float4 *)&a[(ldg_m + ii*32) * k + ldg_k + ib*BLOCK];
            *(float4 *)ldg_b[ii] = *(float4 *)&b[(ldg_n + ii*32) * k + ldg_k + ib*BLOCK];
        }
        ...
        for (int ii = 0; ii < 1; ii++) {
            *(long *)&lds_a[sts_mn + ii*32][SWZ(sts_mn, sts_k    )] = *(long *)&ldg_a[ii][0];
            *(long *)&lds_a[sts_mn + ii*32][SWZ(sts_mn, sts_k + 8)] = *(long *)&ldg_a[ii][8];
            *(long *)&lds_b[sts_mn + ii*32][SWZ(sts_mn, sts_k    )] = *(long *)&ldg_b[ii][0];
            *(long *)&lds_b[sts_mn + ii*32][SWZ(sts_mn, sts_k + 8)] = *(long *)&ldg_b[ii][8];
        }
