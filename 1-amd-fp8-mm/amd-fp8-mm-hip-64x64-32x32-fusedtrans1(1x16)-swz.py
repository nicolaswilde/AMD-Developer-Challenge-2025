...
__global__ void custom_kernel(const __hip_fp8_e4m3_fnuz* __restrict__ a,
    ...
    int sts_k  = threadIdx.y * 16 + threadIdx.x / 4;
    int sts_mn = (threadIdx.x % 4) * 16;
    ...
    for (int ib = 0; ib < k/BLOCK; ib++) {
        ...
        for (int ii = 0; ii < 2; ii++) {
            *(float4 *)ldg_a[ii] = ldg_m < m ? *(float4 *)&a[(ldg_k + ii*64 + ib*BLOCK) * m + ldg_m] : 0;
            *(float4 *)ldg_b[ii] = ldg_n < n ? *(float4 *)&b[(ldg_k + ii*64 + ib*BLOCK) * n + ldg_n] : 0;
        }
